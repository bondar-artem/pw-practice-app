...
test('test', async ({ page }) => {
  ...
  await page1.getByRole('textbox', { name: ' CÓDIGO DO CLIENTE' }).fill('2106');
  await page1.getByRole('textbox', { name: ' NOME DO USUÁRIO' }).click();
  await page1.getByRole('textbox', { name: ' NOME DO USUÁRIO' }).fill('juridico');
  ...
  await page1.getByRole('button', { name: 'ESTATÍSTICAS DE ATENDIMENTO' }).click();
